...
*** CORRECTION (Java 8 onwards) ***
>> "method area" above is conceptual. In real HotSpot JVM (Java 8+):
   - PermGen was REMOVED, replaced by Metaspace (native memory, not heap)
   - Metaspace stores >> class metadata, method bytecode, runtime constant pool
   - Static variables themselves actually live on HEAP, as part of the Class object
   >> so: static var value >> Heap (inside Class object) | class metadata >> Metaspace
...
*** MISSING >> Effectively Final (Java 8+) ***
>> a local variable that is assigned only once (never reassigned) is "effectively final"
>> effectively final local variables can be used inside a lambda / anonymous inner class
   WITHOUT explicitly writing the final keyword
   int base = 10;
   Runnable r = () -> System.out.println(base); // base is effectively final, allowed
   // if base is reassigned anywhere after this, compiler throws error at lambda usage
...
Heap >>
Instance K_Variables , Objects , Arrays , Object Fields || Per Objects
*** MISSING >> Static variables (as part of Class object) also live here in HotSpot Java 8+
...
*** MISSING >> Java 8+ renamed/replaced this as METASPACE (native memory, not heap) - PermGen removed
Pc Register >> current instruction for thread
*** MISSING >> Native Method Stack >> used for native (JNI) method calls || Per thread
...
Processing Data: K_Variables are used to manipulate or transform the data as needed.
Storing Data: Processed data can be stored back into the database.


=========================================================================================================================================
                                MISSING >> JAVA 17 FEATURES RELATED TO VARIABLES
...
1. var keyword (Java 10+) >> local variable type inference
   ...
   Rules :-
   >> only for local variables, enhanced-for loop variables, try-with-resources
   >> also usable for lambda parameters (Java 11+) >> (var a, var b) -> a+b
   >> must have initializer >> var x; is illegal
   >> cannot initialize with null alone (no type to infer)
   >> cannot be used for fields, method parameters(regular), or return types
   >> type fixed at compile time - this is inference, NOT dynamic typing


2. Pattern Matching instanceof (Java 16+, standard in 17)
   Before:
   ...
   >> can combine with && >> if(obj instanceof String s && s.length()>5){ }


3. Records (Java 16+, standard in 17)
   record Employee(int id,String name){}
   >> auto generates :- private final fields, canonical constructor,
                         accessors id()/name() (NOT getId()/getName()),
                         equals(), hashCode(), toString()
   >> records are implicitly final, cannot extend another class, CAN implement interfaces
...
4. Text Blocks (Java 15+, standard in 17)
   String sql = """
   SELECT *
   FROM employee
   """;
   >> multi-line strings without escaping quotes
   >> incidental leading whitespace stripped based on closing """ position


5. Sealed Classes (Java 17)
   sealed class Shape permits Circle, Rectangle {}
   >> restrict inheritance - only permitted classes can extend
   >> child classes must be :- final / sealed / non-sealed
   >> works well with pattern matching for exhaustiveness checks
...
=========================================================================================================================================
    ...
    * */
    ...
    public static void main(String[] args){
        K_Variables v1 = new K_Variables();
        v1.x = 1000;
        v1.y = false;
        K_Variables v2 = new K_Variables();
